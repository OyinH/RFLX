...
- P95 target: ~13 seconds end-to-end (P50 ~4.5s). **Revised, not chased further — a deliberate decision, documented in `docs/rflx_PRD.md`'s Success Metrics note, not an unmet target.** Measured live against the full `specs/08-eval-harness.md` suite (100 cases, production build, real Prompt Shield configured): P95 = 12.9s, P50 = 4.5s. Root cause: the number of sequential reasoning-model round-trips the investigator's loop makes, not request-handling or infra overhead — `reasoning_effort` is already at its floor (`"low"`, `specs/04-investigator.md`). Two structural fixes (parallelizing `risk_classifications`/`incidents` inserts in `recordDecision`; overlapping the eager medications lookup with Prompt Shield) moved P50 meaningfully (~18% faster) but not P95 — the remaining floor is that a single reasoning-model call costs 2-4s by itself, and some cases need two. Closing that gap further would mean a faster non-reasoning model or a deterministic pre-filter for "obviously benign" cases — both of which undercut `docs/rflx_PRD.md` §3.5's argument for agentic investigation over a rules engine, and a pre-filter specifically creates a new bypass surface (a crafted-to-look-benign injection skipping investigation entirely). Not pursued in this pass; the target was revised to reflect the real cost of the reasoning step instead. Separately, this investigation surfaced and fixed a real reliability bug along the way: two requests silently stalled 11-12 minutes despite an already-configured client timeout (`specs/04-investigator.md`'s Client Timeout section) — now hard-capped independent of the OpenAI SDK's own timeout handling. The two metrics that actually validate the hypothesis, catch rate and false-positive rate, both clear their targets cleanly and held steady at 100%/0% throughout.
...
- [x] P95 latency across a representative eval run (`specs/08-eval-harness.md`) is within its revised target (~13s P95, P50 ~4.5s — measured 12.9s / 4.5s, root-caused, documented above and in `docs/rflx_PRD.md`'s Success Metrics table). The original <3s target was replaced, not met — see the Non-Negotiable section above for why.
