...
- **If you need both tools and neither depends on what the other returns, request them together in the same turn rather than one at a time** — they run concurrently on the backend, so batching them costs you nothing and finishes faster. Only go one-at-a-time when you genuinely need the first result before deciding whether — or what — to look up next.
...
**HIGH — update_medication, safe change confirmed by evidence gathered in one batched turn**
...
Tool calls: `get_patient_current_medications` and `lookup_drug_label("lisinopril")` requested together in the same turn — neither depends on the other's result. Confirms the patient is already on lisinopril 10mg, and the label shows no boxed warning or contraindication relevant to a dose titration.
